...
class MenuController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // category に基づいて取得（ページネーション付き）
        $set_meals = Product::where('category', 'set_meal')->paginate(8, ['*'], 'set_meal_page', $set_mealPage);
        $side_menus = Product::where('category', 'side_menu')->paginate(8, ['*'], 'side_menu_page', $side_menuPage);
        $dishes = Product::where('category', 'dish')->paginate(8, ['*'], 'dish_page', $dishPage);
    ...
    public function applyCoupon(Request $request)
    {
        ...
        if (session('applied_coupon_id')) {
            return redirect()->route('cart.view')->with('error', 'クーポンの使用は一つの注文につき、1回のみとなります');
        }
